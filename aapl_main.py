...
class ASParams:
    ...
    soft_flatten_frac: float = 0.7
    soft_flatten_size_frac: float = 0.3
    # NEW: at soft_flatten_frac * flatten_threshold, start crossing the spread with a small
    # partial size (soft_flatten_size_frac * order_size) rather than waiting for the full
    # hard threshold and correcting with the entire order_size at once. Delaying correction
    # via size-skew alone just meant fewer but bigger, more expensive panic-crosses -- this
    # spreads the correction into several cheaper nibbles instead of one large one.
...
def run_backtest(
    ...
    for i in range(n):
        ...
        if i % requote_every_n_events == 0:
            ...
            # inventory_limit / order_size.
            #
            # NOTE: a "soft tier" that crossed with partial size at 0.7*flatten_threshold
            # was tried and reverted -- it fired on nearly every quote cycle whenever
            # inventory sat in that moderate zone (which is often, given inventory_std is
            # comparable to the threshold itself), turning one rare, large correction into
            # many small ones. Since crossing the spread gives up the whole spread
            # regardless of size, there's no economy of scale to "nibbling" -- frequent
            # small crosses cost MORE in aggregate than rare large ones. The size-skew
            # mechanism above (which reduces size on the "wrong" side well before this
            # threshold, without ever paying the spread) remains the effective lever here;
            # this hard override is intentionally a rare last resort.
            forced_bid = False
            forced_ask = False
